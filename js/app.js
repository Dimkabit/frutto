...
function menuInit() {
  ...
  document.addEventListener("click", function(e) {
    if (document.documentElement.hasAttribute("data-fls-menu-open") && !e.target.closest(".menu__body") && !e.target.closest("[data-fls-menu]")) {
      closeMenu();
    }
  });
...
const sidebar = document.querySelector(".sidebar-spollers");
const openBtn = document.querySelector(".open-mobile-filter");
if (sidebar && openBtn) {
  const mq = window.matchMedia("(max-width: 1100px)");
  let isOpen = false;
  const handleMediaChange = () => {
    if (mq.matches) {
      if (!isOpen) sidebar.hidden = true;
    } else {
      isOpen = true;
      ...
      sidebar.style.removeProperty("transition-property");
    }
  };
  handleMediaChange();
  ...
  openBtn.addEventListener("click", () => {
    ...
    if (sidebar.classList.contains("--slide")) return;
    if (!isOpen) {
      isOpen = true;
      slideDown(sidebar);
...
document.addEventListener("DOMContentLoaded", () => {
  const filterNav = document.querySelector(".favorite-nav");
  const items = document.querySelectorAll(".card.--favorite");
  if (!filterNav || items.length === 0) return;
  let currentFilter = "all";
  filterNav.addEventListener("click", (e) => {
    const button = e.target.closest(".navigate");
    if (!button) return;
    filterNav.querySelectorAll(".navigate").forEach((btn) => btn.classList.remove("active"));
    button.classList.add("active");
    currentFilter = button.dataset.filter;
    items.forEach((item) => {
      const itemFilter = item.dataset.filter;
      if (currentFilter === "all" || itemFilter === currentFilter) {
        item.style.display = "";
      } else {
...
document.addEventListener("DOMContentLoaded", function() {
  document.addEventListener("click", function(e) {
    const badge = e.target.closest(".badge--favorite");
    if (!badge) return;
    e.preventDefault();
    if (badge.classList.contains("liked")) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const resetAllButton = filterContainer.querySelector("[data-reset-filter]");
  if (resetAllButton) {
    resetAllButton.addEventListener("click", () => {
      const allCheckboxes = filterContainer.querySelectorAll(".checkbox__input");
      allCheckboxes.forEach((cb) => {
        cb.checked = false;
      });
    });
  }
  const resetGroupLinks = filterContainer.querySelectorAll(".reset-checkbox-group");
  resetGroupLinks.forEach((link) => {
    link.addEventListener("click", (e) => {
      e.preventDefault();
      const group = link.previousElementSibling;
      if (group && group.matches("[data-checkbox-group]")) {
        const checkboxesInGroup = group.querySelectorAll(".checkbox__input");
...
document.addEventListener("DOMContentLoaded", function() {
  document.querySelectorAll(".quantity-selector").forEach(function(selector) {
    ...
    let value = parseInt(selector.getAttribute("data-value")) || min;
    function updateDisplay() {
      display.textContent = value + " кг.";
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const catalogBlock = document.querySelector(".menu-catalog-wrapper");
  const sidebarButtons = document.querySelectorAll(".catalog-block-sidebar button");
  const resultsBlocks = document.querySelectorAll(".results-block");
  if (!catalogLink || !catalogBlock) return;
  function switchCategory(category) {
    sidebarButtons.forEach((button) => {
      button.classList.remove("active");
    });
    const activeButton = document.querySelector(`[data-catalog="${category}"]`);
    if (activeButton) {
    ...
    const activeResultsBlock = document.querySelector(`.results-block[data-catalog="${category}"]`);
    if (activeResultsBlock) {
      activeResultsBlock.classList.add("active");
      activeResultsBlock.style.display = "block";
    }
  }
  function closeCatalog() {
    catalogBlock.classList.remove("active");
    catalogLink.classList.remove("active");
    document.documentElement.classList.remove("open-menu-catalog");
  }
  function openCatalog() {
    catalogBlock.classList.add("active");
    catalogLink.classList.add("active");
    document.documentElement.classList.add("open-menu-catalog");
    activateFirstCategory();
  }
  function activateFirstCategory() {
    if (sidebarButtons.length > 0) {
      const firstCategory = sidebarButtons[0].dataset.catalog;
  ...
  sidebarButtons.forEach((button) => {
    button.addEventListener("click", function() {
      const category = this.dataset.catalog;
      switchCategory(category);
    });
  });
  document.addEventListener("click", function(e) {
    if (!catalogBlock.contains(e.target) && !catalogLink.contains(e.target) && catalogBlock.classList.contains("active")) {
      closeCatalog();
    }
  });
  document.addEventListener("keydown", function(e) {
    if (e.key === "Escape" && catalogBlock.classList.contains("active")) {
      closeCatalog();
    }
  });
  if (catalogBlock.classList.contains("active")) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  popupOpeners.forEach((button) => {
    button.addEventListener("click", (e) => {
      e.preventDefault();
      const targetId = button.getAttribute("data-popup").replace("#", "");
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      const openPopup2 = document.querySelector('.popup:not([aria-hidden="true"])');
      if (openPopup2) closePopup(openPopup2);
    }
  });
  function openPopup(popup) {
    ...
    if (popupBody) {
      const firstInput = popupBody.querySelector('input, textarea, button, [tabindex="0"]');
...
function initSliders() {
  if (document.querySelector(".favorite-nav__slider")) {
    new Swiper(".favorite-nav__slider", {
  ...
  if (document.querySelector(".clients-block__slider")) {
    new Swiper(".clients-block__slider", {
      // <- Указываем класс нужного слайдера
      // Подключаем модули слайдера
      // для конкретного случая
      modules: [Navigation, Pagination],
      ...
      // Кнопки "влево/вправо"
      navigation: {
        prevEl: ".clients-block-button-prev",
        nextEl: ".clients-block-button-next"
      },
      // Брейкпоинты
      breakpoints: {
        0: {
  ...
  if (document.querySelector(".main-blog__slider")) {
    new Swiper(".main-blog__slider", {
      // <- Указываем класс нужного слайдера
      // Подключаем модули слайдера
      // для конкретного случая
      modules: [Navigation, Pagination],
  ...
  if (document.querySelector(".main-review__slider")) {
    new Swiper(".main-review__slider", {
      ...
      speed: 500,
      pagination: {
        el: ".main-review-pagination",
        clickable: true
      },
      // Кнопки "влево/вправо"
      navigation: {
        prevEl: ".main-review-prev",
        nextEl: ".main-review-next"
      },
      // Брейкпоинты
      breakpoints: {
        0: {
  ...
  if (document.querySelector(".images-product")) {
    ...
    new Swiper(".images-product__slider", {
      ...
      spaceBetween: 20,
      speed: 600,
      // Add these for better loop handling:
      loopAdditionalSlides: 2,
      loopPreventsSlide: false,
      loopedSlides: 4,
      // Should match thumbs visible count
      on: {
        init: function() {
          document.querySelector(".lot-block__wrapper")?.classList.remove("loading");
